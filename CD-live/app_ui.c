...
#define TMP_STRING_LEN 125 	/* this number must be larget than the biggest
				 single string in any database structure*/

/* the action can be done in the menu */
typedef enum {
  mo_invalid,
  mo_add_cat,
  mo_add_tracks,
...
/* the local funciton in these program */
...
/* The main function for app_user interface */
void main(int argc , char *argv[])
{
  ...
  /* after the main while  exit database */
  database_close();
  exit(EXIT_SUCCESS);
} /* main match */
...
static void announce(void)
{
  printf("\n\nWelcome to the demostration CD catalog database program\n");
}
...
/* do not use gets cause this method may lead to buffer overflow */
static int enter_new_cat_entry(cdc_entry *entry_to_update)
{
  cdc_entry new_entry;
  char tmp_str[TMP_STRING_LEN + 1];
...
static void enter_new_track_entries(const cdc_entry *entry_to_add_to)
{
  ...
  while(1) {
    ...
    strip_return(tmp_str);
    if (strlen(tmp_str) == 0) {
      if (existing_track_.catalog[0] == '\0') {
    ...
    /* the real code to add new track or refresh existing track*/
...
static void count_all_entries(void)
{
  ...
  do {
	...
	if (cdt_found.catalog[0]) {
      ...
      }	/* inner do match */ while (cdt_found.catalog[0]);
    }
  } /* outer do match */ while (cdc_found.catalog[0]);
